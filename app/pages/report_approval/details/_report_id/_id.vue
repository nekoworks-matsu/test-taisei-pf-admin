      ...
      <div class="ts_content_head">
        <div class="button_header">
          ...
          <div class="button_header_btn">
            <button type="button" class="btn btn-info global_btn" @click="onTransitionButton('/report_approval/edit/' + $route.params.report_id + '/' + item.id)">編集</button>
          </div>
        </div>
      </div>
    </section>
    ...
    <!-- <section class="content-header">
      <h1>
        ...
        <div class="pull-right">
          <button type="button" class="btn btn-info width_140" @click="onTransitionButton('/report_approval/edit/' + $route.params.report_id + '/' + item.id)">編 集</button>
        </div>
      </h1>
    </section> -->
          ...
          <form class="form-horizontal" action="/pdf" name="report_form">
            <div class="box-body">
              <div class="margin_bottom_30">
                <button type="button" class="btn btn-primary global_btn" @click="outPdf" v-bind:disabled="isDisabledPDF">PDFダウンロード</button>
              </div>

              <div class="form-group form_box_group">
                <div class="form_box_group_title">
                  <label class="control-label">報告書承認ID</label>
                </div>
            ...
            <div class="box-footer form_button_flex flex_center border_top">
              <button type="button" class="btn btn-default global_btn form_button_flex_item" @click="onTransitionButton('/report_approval/' + $route.params.report_id)">戻る</button>
            </div>
            <!-- /.box-footer -->
          </form>
        </div>
...
export default {
  ...
  methods: {
    ...
    getApproveReportSections() {
      ...
      for (var i = 0; i < this.reportSections.length; i++) {
        if (this.reportSections[i].reportObjectDefinition != undefined) {
          report[i].reportObjectDefinition.reportObjectGroupDefinition = this.reportSections[i].reportObjectDefinition.reportObjectGroupDefinition;
          report[i].reportObjectDefinition.reportFieldDefinitions = this.reportSections[i].reportObjectDefinition.reportFieldDefinitions;
    ...
    getReportData(reportObjectDefinition, val, isDefault) {
      if (isDefault) {
      ...
      // var data = {id: reportObjectDefinition.reportObjectDefinition.id, label: reportObjectDefinition.reportObjectDefinition.name, sort: reportObjectDefinition.reportObjectDefinition.sort, columns: this.getReportFiledDefinition(reportObjectDefinition), data_list: data_list,image_list: image_list};
      return data;
    },
    onImgClick(tag){
    ...
    getReportSections(operationId, val) {
      ...
      reportObjects.majoritems.forEach(function(category) {
        //that.groupNames.push({"groupName": category.name, "columnId": category.reportObjectDefinitions[0].sort})
        category.reportObjectDefinitions.forEach(function(obj, index) {
          var countValue = that.getReportObjectCount(obj.id, val);
          that.countList.push({label: obj.name, sort: obj.sort, count: countValue});
          if (that.isDisplay) {
            reportData = {id: obj.id, label: obj.name + ' 【報告事項無し】', sort: obj.id};
          }
          if (countValue == 0) {
            if (localStorage.getItem('display_object_if_empty') == "true") {
              reportData = {id: obj.id, label: obj.name + ' 【報告事項無し】', sort: obj.id};
            ...
            if (category.reportObjects != undefined) {
              that.saveFloorData(obj, category.reportObjects[index][0].reportObjectDefinitionId);
            }
            
            reportData = that.getReportData(that.getReportObjectDefinition(obj.id), that.getReportObject(obj.id, val), true);
          }
    ...
    saveFloorData(value, id) {
      var reportObject = this.approveReportReportObjects.filter(val => val.approveReportObject.reportObjectDefinitionId == id);
      var find = value.reportFieldDefinitions.find(val => val.type == 9);
      if (find) {
        this.floorDatas.push({reportObjectDefinition: value, reportObjects: reportObject});
      }
    },
    ...
    convertReportList(reportSections, reportList) {
      ...
      for (var i = 0; i < reportList.majoritems.length; i++) {
        ...
        for (var j = 0; j < filter.length; j++) {          
          reportObjectDefinitions.push({id: filter[j].reportObjectDefinition.id, 
                                        name: filter[j].reportObjectDefinition.name, 
                                        operationCategoryId: filter[j].reportObjectDefinition.operationTypeId, 
                                        path: '/majoritems/' + filter[j].reportObjectDefinition.id, 
                                        sort: filter[j].reportObjectDefinition.sort,
                                        reportFieldDefinitions: filter[j].reportObjectDefinition.reportFieldDefinitions});
          reportObjects.push(filter[j].reportObjects);
        }
        convertReportList.majoritems[i].reportObjectDefinitions = reportObjectDefinitions;
        convertReportList.majoritems[i].reportObjects = reportObjects;
      }
    ...
    floorSortForAsc(underGround, ground, uniq, other, fieldId) {
      underGround.sort(function(a, b){
        var finda = a.approveReportObject.approveReportFields.find(val => val.reportFieldDefinitionId == fieldId);
        var findb = b.approveReportObject.approveReportFields.find(val => val.reportFieldDefinitionId == fieldId);
        if (parseInt(finda.approveReportFieldFloors[0].floor.name) > parseInt(findb.approveReportFieldFloors[0].floor.name)) {
          return -1;
        }
        if (parseInt(finda.approveReportFieldFloors[0].floor.name) < parseInt(findb.approveReportFieldFloors[0].floor.name)) {
          return 1;
    ...
    isFloorSort(id, fieldId, isAsc) {
      ...
      var underGround = find.reportObjects.filter(val => {
        var find2 = val.approveReportObject.approveReportFields.find(value => value.reportFieldDefinitionId == fieldId);
        if (find2.approveReportFieldFloors != undefined && find2.approveReportFieldFloors[0].floor.type == 2) {
          return true;
        }
      });
      var ground = find.reportObjects.filter(val => {
        var find1 = val.approveReportObject.approveReportFields.find(value => value.reportFieldDefinitionId == fieldId);
        if (find1.approveReportFieldFloors != undefined && find1.approveReportFieldFloors[0].floor.type == 1) {
          return true;
        }
      });
      var uniq = find.reportObjects.filter(val => {
        var find3 = val.approveReportObject.approveReportFields.find(value => value.reportFieldDefinitionId == fieldId);
        if (find3.approveReportFieldFloors != undefined && find3.approveReportFieldFloors[0].floor.type == 3) {
          return true;
        }
      });
      var other = find.reportObjects.filter(val => {
        var find4 = val.approveReportObject.approveReportFields.find(value => value.reportFieldDefinitionId == fieldId);
      ...
      var data = this.getReportData({reportObjectDefinition: find.reportObjectDefinition}, result, false);
      this.list.splice(this.list.findIndex(({id}) => id == data.id), 1, data);
    },
  },
  created() {
    const key = (parseInt(this.$route.params.report_id) == 1) ? "Security:ApproveReport:view" : "Cleaning:ApproveReport:view";
