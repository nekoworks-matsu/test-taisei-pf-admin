...
export default {
  ...
  data() {
    return {
      param: {
        ...
        api: '/business-report',
        icon: 'fa-user',
        success: '/business_report/'+this.$route.params.operation_id +'/'+this.$route.params.business_report_id,
        item:{
          searchDate: new Date(),
        ...
        list: {
          ...
          add: '/business_report/'+ this.$route.params.operation_id + '/' + this.$route.params.business_report_id +'/add/',
          read: '/business_report/' + this.$route.params.operation_id + '/' + this.$route.params.business_report_id +'/details/',
        }
  ...
  methods: {
    setMajorItemsHeader(val) {
      ...
      var daily_data = '';
      val.businessReportFieldDefinitions.forEach(function(obj) {
        switch (obj.type) {
          case 1: //NOTE:text
            columns_list.push({
              name: obj.name,
    ...
    addRequest() {
      ...
      for (var i = 0; i < Object.keys(this.item).length; i++) {
        ...
        var index = Number(indexes[1]) - 1;
        
        if (data[i].match('textValue')) {
          fields.push({
            fieldIndex: Number(data[i].slice(10)),
            textValue: this.item[data[i]] == undefined? '':this.item[data[i]],
            businessReportFieldDefinitionId:  this.param.reportFieldDefinitionIds[index].reportFieldDefinitionId
          })
      ...
      if (this.imageColumn != null){
        if (this.isPreview){
          ...
          fields.push({
            ...
            textValue: images,
            businessReportFieldDefinitionId: this.imageColumn.reportFieldDefinitionId
          })
        }else{
          fields.push({
            fieldIndex: this.imageColumn.fields,
      ...
      if (this.videoColumn != null){
        if (this.isVideoPreview) {
          fields.push({
            ...
            businessReportFieldDefinitionId: this.videoColumn.reportFieldDefinitionId
          });
        }else{
          fields.push({
            fieldIndex: this.videoColumn.fields,
      ...
      var send_data = {
        ...
        reportedAt: "2022-04-11T11:21:00.000Z",
        businessReportDefinitionId: this.param.reportObjectDefinitionId,
      ...
      this.req(this.param.api, 'post', send_data, (err, res) => {
        if (err) {
          ...
          that.$router.push("/business_report/" + this.$route.params.operation_id + '/' + res.body.businessReportDefinition.id);
        }
      })
    },
    onAddButtonClick() {
    ...
    getReportExtractFieldId(fields) {
      var find = fields.find(val => val.reportExtractField == true)
      return find.id;
    },
    setSendData(val) {
      var extractFieldId = this.getReportExtractFieldId(val.businessReportFieldDefinitions);
      return {
        "businessReportDefinitionId": val.id,
        "businessReportFieldDefinitionId":extractFieldId,
        "order": [
          {
            "businessReportFieldDefinitionId": extractFieldId,
            "direction": "DESC",
    ...
    search(where) {
      ...
      // this.offset = 0;
      var search_field = 0;
      this.onSearch('/business-report-definitions/' +this.$route.params.business_report_id +'/business-report-field-definitions', null, null, (val1) => {
        ...
        send_data = this.setSendData(val1);
        this.onSearch('/business-report', send_data, null, (val) => {
          ...
          this.param.list.columns = columns_list;
          this.param.items = this.getItemList(val.businessReports, columns_list, this.members);
  ...
  created() {
    ...
    this.checkDisplayPermission(key,() => {
      this.item = {};
      let api_for_list = '/business-report-definitions/' +this.$route.params.business_report_id + '/business-report-field-definitions'
      this.getBusinessReportHeader(api_for_list);
