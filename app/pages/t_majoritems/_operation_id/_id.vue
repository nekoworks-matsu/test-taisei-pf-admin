  ...
  <div align="left">
    ...
    <div class="content ctm_subtitle" v-bind:style="{'margin-top':tabletHeight+'px'}">
      ...
      <div class="box-header box_head t_box_head">
        <button type="button" class="btn global_btn global_btn_border t_main_button" style="display: block !important; font-size: 16px;" @click="onAddButton(param.list.add)">登録</button>
      </div>
      <!-- <div class="box-solid"> -->
        <!-- Main content -->
      <template>
        <div class="box_search_utility t_box margin_bottom_0">
           <div class="t_box_head">
          ...
          <div class="box-body no-paddin">
            <form name="searchForm" class="form-horizontal" style="margin:0;" v-on:submit.prevent="search" autocomplete="off">
              <div class="box-body">
                <div class="t_form_parts t_form_parts_short" v-for="form in searchForm" v-if="form.availableForSearch">
                  <template v-if="form.type==2">
                    <div class="t_form_parts_title">
                      <label>{{param.search_item.label}}</label>
                    </div>
                    <!-- <label class="col-sm-10 pull-left font_18">{{param.search_item.label}}</label> -->
                    <div class="t_form_parts_field">
                      <div class="input_min_height margin_bottom_15">
                        <input class="t_checkbox_position t_radio" type="radio" name="searchRadios" id="searchRadios1" value="option1" v-model="item.radioButtonState">
                        <label class="search_type font_16" for="searchRadios1">年月日</label>
                        <div class="majoritem_search_ui padding_left_30">
                          <div class="t_date_select">
                            <vuejs-datepicker class="form-control input_date_picker _t_form_parts_content" :clear-button="false" :format="DatePickerFormat" :language="language" name="datepicker" id="searchDate" v-model="item.searchDate" v-bind:placeholder="'From YYYY/MM/DD'"></vuejs-datepicker>
                          </div>
                          <div class="t_date_select_btn">
                            <button type="button" class="btn btn-default btn-lg btn_width50" @click="setDay(-7)"><i class="fa fa-angle-double-left"></i><span>　1週間前</span></button>
                            <button type="button" class="btn btn-default btn-lg btn_width50" @click="setDay(-1)"><i class="fa fa-angle-left"></i><span>　1日前</span></button>
                          </div>
                          <div class="t_date_select_btn padding_top_7">
                            <button type="button" class="btn btn-default btn-lg btn_width50" @click="setDay(1)"><span>1日後　</span><i class="fa fa-angle-right"></i></button>
                            <button type="button" class="btn btn-default btn-lg btn_width50" @click="setDay(7)"><span>1週間後　</span><i class="fa fa-angle-double-right"></i></button>
                          </div>
                        </div>
                      </div>
                      <div class="input_min_height">
                        <input class="t_checkbox_position t_radio" type="radio" name="searchRadios" id="searchRadios2" value="option2" v-model="item.radioButtonState">
                        <label class="search_type font_16" for="searchRadios2">範囲</label>
                        <div class="majoritem_search_ui padding_left_30">
                          <div class="t_majoritem_date_range padding_bottom_15">
                            <vuejs-datepicker class="form-control input_date_picker _t_form_parts_content" :value="startShowAt.value" :clear-button="true" :format="DatePickerFormat" :language="language" name="datepicker" id="startShowAt" @input="inputDate($event, form.fieldIndex, form.type, true, form.id)" v-bind:placeholder="'From YYYY/MM/DD'"></vuejs-datepicker>
                            <div class="padding_top_7">
                              <select class="form-control t_date_time_hours _t_form_parts_content" id="start_hh" v-model="start_hh">
                                <option v-bind:value="opt.value" v-for="opt in param.hh_op">{{opt.name}}</option>
                              </select>
                              <select class="form-control t_date_time_hours _t_form_parts_content" id="start_mm" v-model="start_mm">
                                <option v-bind:value="opt.value" v-for="opt in param.mm_op">{{opt.name}}</option>
                              </select>
                            </div>
                          </div>
                          <div class="t_majoritem_date_range padding_bottom_15">
                            <vuejs-datepicker class="form-control input_date_picker _t_form_parts_content" :value="endShowAt.value" :clear-button="true" :format="DatePickerFormat" :language="language" id="endShowAt" name="datepicker" @input="inputDate($event, form.fieldIndex, form.type, false, form.id)" v-bind:placeholder="'To YYYY/MM/DD'"></vuejs-datepicker>
                            <div class="padding_top_7">
                              <select class="form-control t_date_time_hours _t_form_parts_content" id="end_hh" v-model="end_hh">
                                <option v-bind:value="opt.value" v-for="opt in param.hh_op">{{opt.name}}</option>
                              </select>
                              <select class="form-control t_date_time_hours _t_form_parts_content" id="end_mm" v-model="end_mm">
                                <option v-bind:value="opt.value" v-for="opt in param.mm_op">{{opt.name}}</option>
                              </select>
                            </div>
                          </div> 
                  ...
                  <template v-if="(form.type==1)||(form.type==3)||(form.type==6)">
                    ...
                    <!-- <label class="col-sm-10 pull-left font_16">{{form.name}}</label> -->
                    <div class="t_form_parts_field">
                      <p></p>
                      <input type="text" class="form-control _t_form_parts_content" :value="getValue(form.fieldIndex)" @input="inputSearch($event.target.value, form.fieldIndex, form.type, form.id)">
                    </div>
                  </template>
                  <template v-if="(form.type==4)||(form.type==5)">
                    ...
                    <div class="t_form_parts_field padding_top_5">
                      <select class="form-control _t_form_parts_content" :value="getValue(form.fieldIndex)" @input="inputSearch($event.target.value, form.fieldIndex, form.type, form.id)">
                        <option></option>
              ...
              <div class="box-footer clearfix">
                <div class="no-margin" style="text-align: center;">
                  <button type="submit" class="btn btn-info global_btn t_main_button">検 索</button>
                </div>
              </div>
              <!-- /.box-footer -->
            </form>
      ...
      <list-view @onImgClick="onImgClick" :isTablet="param" :isRemoveAdd="isRemoveAdd" :list="items" :param="param.list" :count="count" :limit="limit" :offset="offset" :onSearch2="search2"></list-view>
        <!-- /.content -->
      <!-- </div> -->
    </div>
  <template>
    <modal-img-view　@toDetails="toDetails" :isTablet="'true'"　ref="child" :id="'modal'" :title="'拡大プレビュー'" :transition="true"></modal-img-view>
  </template>
  <template>
...
export default {
    ...
    return {
      ...
      param: {
        ...
        list: {
          ...
          add: '/t_majoritems/'+ this.$route.params.operation_id + '/' + this.$route.params.id +'/add/',
          read: '/t_majoritems/' + this.$route.params.operation_id + '/' + this.$route.params.id +'/details/',
        }
      }
    }
  },
  ...
  methods: {
    ...
    setConditions(api, start, end, search_field) {
      ...
      for (var i = 0; i < keys.length; i++) {
        if (this.input[keys[i]].value != "") {
          ...
          conditions.push({
            reportFieldDefinitionId: this.input[keys[i]].id, 
            inputType: inputType,
     ...
     * リクエストデータ取得
     */
    getSendData(api, searchField, where, startShowAt, endShowAt) {
      var sendData = {
    ...
    search(where) {
      ...
      this.onSearch('/report-object-definitions/' +this.$route.params.id +'/report-field-definitions', null, null, (val1) => {
        ...
        this.setSearchForm(val1.reportFieldDefinitions);
        ...
        send_date = this.getSendData(this.param.api, search_field, where, startShowAt, endShowAt);
        ...
        if (!this.param.list.disablePaging) {
          this.onCount(this.param.api, count_cond, (count) => {
            ...
            if (this.count > this.countLimit) {
              this.alert.title = '該当件数が' + this.countLimit + '件を超えています';
              this.alert.message = '検索条件を変更し再度検索してください。';
              this.alert.note = '';
    ...
    search2(offset) {
      ...
      this.onSearch('/report-object-definitions/' +this.$route.params.id +'/report-field-definitions', null, null, (val1) => {
        ...
        var endShowAt   = this.getShowAtDateTime(this.endShowAt.value, this.end_hh, this.end_mm, "end");
        if(this.getDateError() != '') return; 
        send_data = this.getSendData(this.param.api, search_field, this.where, startShowAt, endShowAt);
    ...
    setMajoritemsTitle() {
      ...
      this.param.title = this.getMajorItemsTitle(report_list) + '一覧';
    },
    setDay(day) {
    ...
    setSearchItem() {
      ...
      if (JSON.parse(searchItem) != null) {
        ...
        if (this.param.api == '/report-objects/search-by-business-date') {
          ...
          //var start = {date: this.startShowAt.value, hour: this.start_hh, minute: this.start_mm};
        //var end = {date: this.endShowAt.value, hour: this.end_hh, minute: this.end_mm}
        }
      }
    },
    onAddButton(path) {
  ...
  mounted() {
    //this.init();
    localStorage.setItem("privious_page", this.$route.params.id);
