      ...
      <div class="ts_content_head">
        <div class="button_header">
          <section>
            <div class="ts_breadcrumb">
              <ol>
                ...
                <li v-if="isHeadquartersMode">{{param.title}}詳細</li>
                <li v-else>{{param.title}}情報</li>
          ...
          <!-- <div class="button_header_btn">
            <button type="button" class="btn btn-info global_btn" @click="onTransitionButton('/majoritems_setting/' + $route.params.operation_id + '/details/' + displayItem.id)">キャンセル</button>
          </div> -->
        </div>
      </div>
    </section>
    <!-- <section class="content-header">
      <h1>
        ...
        <div class="pull-right">
          <button type="button" class="btn btn-default width_140" @click="onTransitionButton('/majoritems_setting/' + $route.params.operation_id + '/details/' + displayItem.id)">キャンセル</button>
        </div>
      </h1>
    </section> -->
          ...
          <div class="box-body">
            <form class="form-horizontal" style="margin:0;">
              ...
              <div class="box-footer form_button_flex flex_right border_top">
                <button type="button" class="btn btn-danger global_btn form_button_flex_item margin_right_auto" @click="onDeleteFunc()" v-if="isHeadquartersMode">削除</button>
                <button type="button" class="btn btn-default global_btn form_button_flex_item" @click="onTransitionButton('/majoritems_setting/' + $route.params.operation_id + '/details/' + displayItem.id)">キャンセル</button>
                <button type="button" class="btn btn-warning global_btn form_button_flex_item" @click="onUpdateButtonClick">保存</button>
              </div>
              <!-- /.box-footer -->
            </form>
          </div>
    ...
    methods: {
      ...
      getFloorsInfo(data) {
        this.onSearch('/buildings/' + parseInt(this.displayItem.buildingId), null, null, (val) => {
          ...
          this.buildingOperationReports = val.buildingOperationReports;
      ...
      getRequestData() {
        var request = {"buildingId": this.displayItem.buildingId, "operationCategoryId": parseInt(this.displayItem.operationCategoryId), "autoCreationScheduleReportObject": []};
      ...
      deleteFunc() {
        this.onDelete('/autocreation-schedule-operation-category', '/majoritems_setting/'+parseInt(this.$route.params.operation_id))
      },
      onDeleteFunc() {
        this.alert.title = '';
